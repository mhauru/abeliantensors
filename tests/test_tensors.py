...
for cls in classes:
    ...
    if pars["test_splitting_and_joining"]:
        ...
        for iter_num in range(pars["n_iters"]):
            ...
            batch_new_indices = [
                remaining_indices.index(i) for i in batch_firsts
            ]
            dim_batches = [
                [T.shape[i] for i in batch] for batch in index_batches
            ]
    ...
    if pars["test_product"]:
        for iter_num in range(pars["n_iters"]):
            ...
            if n:
                i_list = list(
                    np.random.choice(len(shp1), size=n, replace=False)
                )
            else:
                i_list = []
            ...
            if n:
                j_list = list(
                    np.random.choice(len(shp2), size=n, replace=False)
                )
            else:
                j_list = []
            for k in range(n):
    ...
    if pars["test_eig"]:
        for iter_num in range(pars["n_iters"]):
            ...
            S_np_eig, U_np_eig = eig(
                T_np, i_list, i_list_compl, hermitian=True
            )
            S_eig_np, U_eig_np = S.to_ndarray(), U.to_ndarray()
    ...
    if pars["test_ncon_svd_ncon"]:
        for iter_num in range(pars["n_iters"]):
            ...
            tensors = []
            np_tensors = []
            for shape, qhape, dirs, charge in zip(
                shapes, qhapes, dirss, charges
            ):
